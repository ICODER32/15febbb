...
if ('serviceWorker' in navigator) {
  ...
  wb.addEventListener('activated', (event) => {
    if (!event.isUpdate) {
      logger('Service worker activated for the first time.');
    } else {
      logger('Service worker activated because of an update.');
    }
  });

  wb.addEventListener('waiting', (event) => {
    logger('A new service worker has installed and is waiting to activate.');
  });

  wb.register().then((registration) => {
    logger('Service worker registered:', registration);
  });
} else {
